...
class Problem:
    ...
    def solve(self):
        ...
        if len(self.y.t) == 2:
            ...
            option = options[
                cutie.select(options, caption_indices=[0], selected_index=1)
            ]
    ...
    def speedAtGround(self):
        clear(SIZE)
        print(
            "Speed on impact with ground is {0:.2f}m/s\n".format(
                sqrt(self.x.v1 * self.x.v1 + self.y.v2 * self.y.v2)
            )
        )
    ...
    def velocityAtGround(self):
        clear(SIZE)
        print(
            "Velocity on impact with ground is {0:.2f}m/s offset {1:.2f} degress down\n".format(
                sqrt(self.x.v1 * self.x.v1 + self.y.v2 * self.y.v2),
                degrees(atan(self.y.v2 / self.x.v1)),
            )
        )
    ...
    def xyT(self):
        ...
        print(
            "It has traveled {0:.2f}m in the x direction and is {2:.2f}m off the ground at {1}s\n".format(
                x.s, x.t, self.height + y.s
            )
        )
    ...
    def xyD(self):
        ...
        if option == "Find X from Y":
            ...
            if answers:
                if len(answers) == 2:
                    correct = cutie.select(
                        ["Which is correct for time?"] + list(answers),
                        caption_indices=[0],
                    )
                    toRemove += 3
                else:
            ...
            clear(toRemove)
            print(
                "It will have traved {0:.2f}m in the X direction when it is {1:.2f}m off the ground\n".format(
                    s, t - self.y.s
                )
            )
            ...
            print(
                "It will be {0:.2f}m off the ground when it has gone {1:.2f}m on the X axis\n".format(
                    self.height + s, t
                )
            )
